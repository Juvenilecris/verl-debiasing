...
def process_image(image: dict | Image.Image) -> Image.Image:
    ...
    if "bytes" in image:
        ...
        raw = image["bytes"]
        # Accept bytes/bytearray directly, or list of ints/strings (legacy data)
        if isinstance(raw, (bytes, bytearray)):
            buf = bytes(raw)
        elif isinstance(raw, list):
            # Support [255, 216, ...] or ["255", "216", ...]
            try:
                buf = bytes(int(x) for x in raw)
            except Exception as _:
                raise TypeError("image['bytes'] list must contain ints or int-like strings (0-255)")
        else:
            raise TypeError(f"Unsupported type for image['bytes']: {type(raw)}")
        image["image"] = Image.open(BytesIO(buf))

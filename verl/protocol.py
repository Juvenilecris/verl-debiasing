        ...
        if len(a) != len(b):
            ...
            for key in a:
                if key not in b:
                    result = False
                    break
                if not _deep_equal(a[key], b[key], visited):
                    result = False
                    break
    elif isinstance(a, torch.Tensor):
        # Handle PyTorch tensor comparison
        if a.shape != b.shape:
            result = False
        else:
            try:
                ...
                # Fallback to element-wise comparison if torch.equal fails
                try:
                    result = (a == b).all().item()
                except Exception:
                    # If all else fails, compare as arrays
                    result = np.array_equal(a.detach().cpu().numpy(), b.detach().cpu().numpy())
    else:
        try:
            result = a == b
        except Exception as e:
            # Debug information output when comparison fails
            print(f"=== Comparison Error in _deep_equal ===")

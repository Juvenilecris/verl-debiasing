...
from verl.interactions.waterbird_interaction import WaterbirdInteraction


class TestWaterbirdInteraction:
    """Test cases for WaterbirdInteraction class."""

    def setup_method(self):
        """Set up test environment before each test method."""
        self.config = {"name": "waterbird"}
        self.interaction = WaterbirdInteraction(self.config)

    def test_init(self):
        """Test WaterbirdInteraction initialization."""
        assert self.interaction._instance_dict == {}
        assert self.interaction.config == self.config
        assert self.interaction.name == "waterbird"
    ...
    async def test_generate_response_correct_answer_with_prefix(self):
        ...
        with patch("verl.utils.reward_score.waterbird.compute_score", return_value=1.0):
    ...
    async def test_generate_response_correct_answer_without_prefix(self):
        ...
        messages = [{"role": "assistant", "content": "<box>[12,14,200,210]</box>"}]
        ...
        # assert response == "Your response is correct!"
        assert reward == 1.0
        assert self.interaction._instance_dict[instance_id]["response"] == "<box>[12,14,200,210]</box>"
        print(response)
    ...
    async def test_generate_response_incorrect_answer(self):
        ...
        with patch("verl.utils.reward_score.waterbird.compute_score", return_value=0.0):
            should_terminate, response, reward, metadata = await self.interaction.generate_response(
                instance_id, messages
            )
    ...
    async def test_calculate_score_direct_call(self):
        ...
        with patch("verl.utils.reward_score.waterbird.compute_score", return_value=1.0) as mock_compute:
    ...
    async def test_calculate_score_with_kwargs(self):
        ...
        with patch("verl.utils.reward_score.waterbird.compute_score", return_value=0.0) as mock_compute:
    ...
    async def test_multiple_concurrent_interactions(self):
        ...
        with patch("verl.utils.reward_score.waterbird.compute_score", side_effect=[1.0, 1.0]):
    ...
    def test_inheritance_from_base_interaction(self):
        """Test that WaterbirdInteraction properly inherits from BaseInteraction."""
    ...
    def test_name_attribute_initialization(self):
        ...
        config_with_name = {"name": "custom_waterbird"}
        interaction_with_name = WaterbirdInteraction(config_with_name)
        assert interaction_with_name.name == "custom_waterbird"

        # Test with default name when not provided in config
        config_without_name = {}
        interaction_without_name = WaterbirdInteraction(config_without_name)
...
if __name__=="__main__":
    test=TestWaterbirdInteraction
    test.test_generate_response_correct_answer_without_prefix(test)
